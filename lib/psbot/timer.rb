...
module PSBot
  # Timers are user for exeuting code in the future, either
  # repeatedly or only once.
  #
  # In PSBot, two ways for creating timers are available:
  #
  ...
  class Timer
    ...
    #   will stop. This value will automatically reset after restarting
    #   the timer.
    attr_accessor :shots
    ...
    # @option options [Boolean] :threaded (true) If true, each
    #   invocation will be executed in a thread of its own.
    # @option options [Boolean] :start_automatically (true) If true,
    #   the timer will automatically start after the bot finished
    #   connecting.
    # @option options [Boolean] :stop_automatically (true) If true, the
    ...
    # @return [String]
    def to_s
      "<PSBot::Timer %s/%s shots, %ds interval, %sthreaded, %sstarted, block: %s>" % [@orig_shots - @shots, @orig_shots, @interval, @threaded ? "" : "not ", @started ? "": "not ", @block]
    end
  end
end

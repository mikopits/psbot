...
module PSBot
  ...
  # PSBot. It provides both the methods for users to write their own
  # plugins as well as for the PSBot framework to use them.
  #
  # The {ClassMethods} module, which will get included automatically
  # in all classes that include `PSBot::Plugin`, includes all class
  # methods that the user will use for creating plugins.
  #
  # Most of the instance methods are for use by the PSBot framework
  ...
  module Plugin
    ...
    # need for creating plugins for the PSBot framework: Setting
    ...
    module ClassMethods
      ...
      # @return [Array<:message, :room, :private>] The list of events to react on
      ...
      #
      # @note This is not the same as a {PSBot::Timer} object, which
      ...
      #     events are listed in the {file:docs/events.md list of events}.
      #   @param [Hash] options
      #   @option options [Symbol] :method (:listen) The method to execute
      ...
      #   def some_method
      #     Room("techcode").send("Time.now.to_s")
      #   end
      #
      ...
      #   the timer will automatically start after thebot finished
      #   connecting.
      # @option options [Boolean] :stop_automatically (true) If true,
      ...
      #   a handler?
      # @option options [Array<:match, :listen_to>] :for ([:match, :listen_to])
      ...
      def hook(type, options = {})
        options = {:for => [:match, :listen_to], :method => :hook, :group => nil}.merge(options)
    ...
    def __register_listeners
      self.class.listeners.each do |listener|
        @bot.loggers.debug "[plugin] #{self.class.plugin_name}: Register listener for type `{#{listener.event}}`"
    ...
    def __register_timers
      @timers = self.class.timers.map {|timer_struct|
        ...
        PSBot::Timer.new(@bot, options, &block)
      }
    end
    ...
    def __register_matchers
      ...
      self.class.matchers.each do |matcher|
        ...
        new_handler = Handler.new(@bot,
                                  react_on,
                                  pattern_to_register,
                                  group: matcher.group) do |message, *args|
    ...
    # Provides acces to plugin-specific options.
    #
